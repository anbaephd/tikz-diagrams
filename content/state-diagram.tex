...
\begin{tikzpicture}
\begin{umlstate}[name=method]{Methodolgy}
\begin{umlstate}[name=metadata, fill=red!20]{Metadata}
\node[database,label=below:zotero, database radius=0.25cm, database segment height=0.5cm, name=zotero] {};
\end{umlstate}

\begin{umlstate}[y=-3,name=scripts, fill=red!20]{Scripts}
\node {01, 01, ..., 05};
\end{umlstate}


\begin{umlstate}[x=5,name=query, fill=red!20]{Query}
\node {asd};
\end{umlstate}
